...
public class MoreAge {
    public static ArrayList<WordPos> ageism(Path input) throws IOException {
        ...
        Path expressions = Paths.get(projRoot.toString(),"biased-words", "ageism_expressions_anti-older.txt");
        Path synonymLists = Paths.get(projRoot.toString(),"biased-words", "ageism_anti-older_synonyms.txt");

        ArrayList<WordPos> filtered = Tokeniser.filterExpressions(input, expressions, synonymLists);

        for (WordPos wp: filtered)
            wp.setAgeAntiOlderStat();
